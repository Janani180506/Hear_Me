...
def get_lemma(word_text, upos):
    ...
    if upos == 'VERB':
        ...
        pos_char = 'r'
    try:
        return lemmatiser.lemmatize(word_text.lower(), pos_char)
    except Exception:
        return word_text.lower()

class StanzaWordMock:
...
def en_nlp(text_input):
    ...
    for s in sentences:
        ...
        for token, pos in tagged:
            words_mock.append(StanzaWordMock(token, get_upos(pos)))
        sentences_mock.append(StanzaSentenceMock(s, words_mock))
        
    return StanzaDocMock(sentences_mock)
		...
		for i,(word,fin) in enumerate(zip(words,final)):
			if fin in word.text:
				if(len(fin)==1):
					...
					final[i]=get_lemma(fin, word.upos);
...
def reorder_eng_to_isl(input_string):
	...
	try:
		...
		for word, tag in tagged:
			if tag in ("PRP", "PRP$") or (tag.startswith("NN") and not subjects):
		...
		reordered = subjects + objects + others + verbs
		if len(reordered) == len(input_string):
			return reordered
		return input_string
	except Exception as e:
		print("NLTK SOV reordering fallback failed:", e)
		return input_string
